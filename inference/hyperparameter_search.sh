...
PYPYTHON
    ...
    echo "Running TruthfulQA on full samples..." | tee -a "$LOG_FILE"
    ...
    echo "${run_id},${model},${mode},${temp},${repeat_penalty},${top_p},${top_k},${min_p},${ctx_size},${keep},${batch_size},${ubatch_size},${threads},${ngl},${ctk},${ctv},${flash_attn},${context_shift},${poll_level},${use_mmap},${split_mode},${dry_mult},${freq_penalty},${presence_penalty},${token_limit},${bleurt_score},${accuracy},${avg_prefill_speed},${avg_decode_speed},${avg_total_speed},${runtime}" >> "$RESULTS_CSV"
    update_best_results
    echo "" | tee -a "$LOG_FILE"
}
...
for run_id in $(seq 1 $NUM_TRIALS); do
    ...
    run_configuration "$run_id" "$model" "$mode" "$temp" "$repeat_penalty" "$top_p" "$top_k" "$min_p" "$ctx_size" "$keep" "$batch_size" "$ubatch_size" "$threads" "$ngl" "$ctk" "$ctv" "$flash_attn" "$context_shift" "$poll_level" "$use_mmap" "$split_mode" "$dry_mult" "$freq_penalty" "$presence_penalty" "$token_limit"
