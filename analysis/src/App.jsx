...
const getModelColor = (model, modelList) => {
...
const processData = (data) => {
  ...
  return valid.map(d => ({
    ...d,
    norm_accuracy: (d.accuracy - minAcc) / accRange,
    norm_speed: (d.decode_speed - minSpeed) / speedRange,
    combined: ((d.accuracy - minAcc) / accRange + (d.decode_speed - minSpeed) / speedRange) / 2,
...
function App() {
  ...
  const models = useMemo(() => 
    [...new Set(processedData.map(d => d.model_short))].sort(), 
    [processedData]
  );
  ...
  const modelColors = useMemo(() => {
    const colors = {};
    models.forEach((m, i) => {
      colors[m] = colorPalette[i % colorPalette.length];
    });
    return colors;
  }, [models]);
  ...
  const filteredData = useMemo(() => {
    ...
    if (selectedModel !== 'all') {
      data = data.filter(d => d.model_short === selectedModel);
    }
    if (selectedSource !== 'all') {
      data = data.filter(d => d._source === selectedSource);
    }
    return data;
  ...
  const threadStats = useMemo(() => {
    if (filteredData.length === 0 || !filteredData[0].threads) return [];
    const groups = {};
    filteredData.forEach(d => {
      if (d.threads === undefined) return;
      if (!groups[d.threads]) groups[d.threads] = { threads: d.threads, accuracy: [], speed: [], combined: [] };
  ...
  const tempStats = useMemo(() => {
    ...
    filteredData.forEach(d => {
      ...
      if (!groups[d.temperature]) groups[d.temperature] = { temp: d.temperature, accuracy: [], speed: [], combined: [] };
  ...
  const modelStats = useMemo(() => {
    ...
    dataToUse.forEach(d => {
      if (!groups[d.model_short]) groups[d.model_short] = { model: d.model_short, accuracy: [], speed: [], combined: [] };
  ...
  const topRuns = useMemo(() => {
    return [...filteredData].sort((a,b) => b.combined - a.combined).slice(0, 5);
  }, [filteredData]);
    ...
    return (
      <div style={{
        ...
        <p style={{ color: '#64748b', marginBottom: 32, textAlign: 'center' }}>
          Upload one or more CSV files from your hyperparameter search
        </p>
        ...
        <p style={{ color: '#475569', marginTop: 24, fontSize: '0.85rem' }}>
          Expected columns: run_id, model, accuracy, avg_decode_speed (or decode_speed), temperature, threads, etc.
        </p>
      </div>
    );
  }
    ...
    <div style={{
      ...
      <div style={{ marginBottom: 32, textAlign: 'center' }}>
        ...
        <p style={{ color: '#64748b', fontSize: '1rem' }}>
          {filteredData.length} valid runs • {files.length} file(s) loaded
        </p>
      </div>
      ...
      <div style={{ marginBottom: 40 }}>
        <h2 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: 16, color: '#fbbf24' }}>
          🏆 Top 5 Configurations (Efficiency + Accuracy)
        </h2>
          ...
          {topRuns.map((run, i) => (
            <div key={`${run._source}-${run.run_id}`} style={{
              ...
              <div style={{
                ...
                top: 12,
                right: 12,
                background: i === 0 ? '#fbbf24' : '#475569',
                color: i === 0 ? '#0f172a' : '#e2e8f0',
                borderRadius: 20,
                padding: '4px 12px',
                fontSize: '0.75rem',
                fontWeight: 700
              ...
              <div style={{ fontSize: '1.1rem', fontWeight: 700, color: modelColors[run.model_short], marginBottom: 8 }}>
                {run.model_short}
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, fontSize: '0.85rem' }}>
                <div>
                  <span style={{ color: '#64748b' }}>Score: </span>
                  <span style={{ fontWeight: 700, color: '#fbbf24' }}>{run.combined?.toFixed(3)}</span>
                </div>
                <div>
                  <span style={{ color: '#64748b' }}>Acc: </span>
                  <span style={{ fontWeight: 600 }}>{(run.accuracy * 100).toFixed(0)}%</span>
                </div>
                <div>
                  <span style={{ color: '#64748b' }}>Speed: </span>
                  <span style={{ fontWeight: 600 }}>{run.decode_speed?.toFixed(1)}</span>
                </div>
                {run.temperature !== undefined && <div>
                  <span style={{ color: '#64748b' }}>Temp: </span>
                  <span>{run.temperature}</span>
                </div>}
                {run.threads !== undefined && <div>
                  <span style={{ color: '#64748b' }}>Threads: </span>
                  <span>{run.threads}</span>
                </div>}
                {run.repeat_penalty !== undefined && <div>
                  <span style={{ color: '#64748b' }}>Rep: </span>
                  <span>{run.repeat_penalty}</span>
                </div>}
              </div>
              {files.length > 1 && (
      ...
      <div style={{ 
        ...
        <div>
          <label style={{ color: '#94a3b8', fontSize: '0.85rem', marginRight: 8 }}>Filter by Model:</label>
          <select 
            value={selectedModel}
        ...
        {files.length > 1 && (
          <div>
            <label style={{ color: '#94a3b8', fontSize: '0.85rem', marginRight: 8 }}>Filter by File:</label>
            <select 
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(500px, 1fr))', gap: 24, marginBottom: 32 }}>
        ...
        <div style={{
          ...
          <h3 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: 16, color: '#94a3b8' }}>
            Speed vs Accuracy (size = combined score)
          </h3>
          <ResponsiveContainer width="100%" height={350}>
            <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
              ...
              <XAxis 
                ...
                label={{ value: 'Decode Speed (tok/s)', position: 'bottom', fill: '#94a3b8', fontSize: 12 }}
              />
              <YAxis 
                type="number" 
                dataKey="accuracy" 
              ...
              <Scatter data={filteredData} fill="#3b82f6">
                {filteredData.map((entry, index) => (
                  <Cell 
                    key={`cell-${index}`} 
                    fill={modelColors[entry.model_short] || '#3b82f6'}
                    fillOpacity={0.8}
                  />
                ))}
              </Scatter>
            ...
            <BarChart data={modelStats} layout="vertical" margin={{ top: 10, right: 30, left: 100, bottom: 10 }}>
              ...
              <Bar dataKey="combined" name="Combined Score" radius={[0, 4, 4, 0]}>
                {modelStats.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={modelColors[entry.model] || '#3b82f6'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
            ...
            <ResponsiveContainer width="100%" height={350}>
              <LineChart data={threadStats} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                ...
                <XAxis 
                  ...
                  stroke="#64748b" 
                  tick={{ fill: '#94a3b8', fontSize: 12 }}
                  label={{ value: 'Thread Count', position: 'bottom', fill: '#94a3b8', fontSize: 12 }}
                />
                <YAxis 
                  yAxisId="left"
                  stroke="#10b981" 
                ...
                />
                <Legend />
                <Line yAxisId="left" type="monotone" dataKey="speed" name="Avg Speed (tok/s)" stroke="#10b981" strokeWidth={3} dot={{ r: 6, fill: '#10b981' }} />
                <Line yAxisId="right" type="monotone" dataKey="combined" name="Combined Score" stroke="#f59e0b" strokeWidth={3} dot={{ r: 6, fill: '#f59e0b' }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        )}

        {/* Line: Temperature Impact */}
        {tempStats.length > 1 && (
          <div style={{
            background: 'rgba(30, 41, 59, 0.5)',
            border: '1px solid rgba(148, 163, 184, 0.15)',
            ...
            <ResponsiveContainer width="100%" height={350}>
              <LineChart data={tempStats} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                ...
                <XAxis 
                  ...
                  stroke="#64748b" 
                  tick={{ fill: '#94a3b8', fontSize: 12 }}
                  label={{ value: 'Temperature', position: 'bottom', fill: '#94a3b8', fontSize: 12 }}
                />
                <YAxis 
                  yAxisId="left"
                  stroke="#8b5cf6" 
      ...
      <div style={{
        ...
        <h3 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: 16, color: '#10b981' }}>
          📊 Summary Statistics
        </h3>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
          <div>
            <div style={{ color: '#64748b', fontSize: '0.85rem' }}>Total Valid Runs</div>
          ...
          <div>
            ...
            <div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#8b5cf6' }}>
              {(Math.max(...filteredData.map(d => d.accuracy)) * 100).toFixed(1)}%
            </div>
          </div>
          <div>
            <div style={{ color: '#64748b', fontSize: '0.85rem' }}>Best Speed</div>
            <div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#10b981' }}>
              {Math.max(...filteredData.map(d => d.decode_speed)).toFixed(1)} tok/s
            </div>
          </div>
          <div>
            <div style={{ color: '#64748b', fontSize: '0.85rem' }}>Best Combined</div>
            <div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#fbbf24' }}>
              {Math.max(...filteredData.map(d => d.combined)).toFixed(3)}
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ textAlign: 'center', marginTop: 40, color: '#475569', fontSize: '0.85rem' }}>
        LLM Hyperparameter Analyzer • {files.length} file(s) • {filteredData.length} runs
      </div>
    </div>
  );
}
